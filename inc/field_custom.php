...
namespace GMWooDateProduct\inc\fieldCustom;
...
function gm_display_custom_field()
{
  ...
  if ($showCustom === 'yes') {
    ...
    echo '<div id="gm-woo-date-product-wrapper" data-value="' . (array_key_exists('gm-date-selector', $_POST) ? $_POST['gm-date-selector'] : '') . '" >'
      . '<label>' . __('Desired date for removal', 'gm-woo-date-product') . '<input name="gm-date-selector" type="date" id="gm-datepicker-product" min="' . date('Y-m-d', strtotime(date('Y-m-d') . ' + 2 days')) . '" placeholder="' . __('Choose a date', 'gm-woo-date-product') . '" readonly/></label>'
      . '</div>';
  }
}
